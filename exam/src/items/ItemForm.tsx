...
const ItemForm: React.FC<ItemFormProps> = ({
  ...
  const [energi_Kj, setEnergiKj] = useState<string>(initialData?.energi_Kj?.toString() || '');
  const [fett, setFett] = useState<string>(initialData?.fett?.toString() || '');
  const [protein, setProtein] = useState<string>(initialData?.protein?.toString() || '');
  const [karbohydrat, setKarbohydrat] = useState<string>(initialData?.karbohydrat?.toString() || '');
    ...
    switch (field) {
      case 'name':
        if (!value.match(/^[a-zA-ZæøåÆØÅ0-9\s-]{2,50}$/)) {
          error = 'Name must be 2-50 characters long and can include letters, numbers, spaces, and hyphens.';
        }
        break;
      case 'food_Group':
        if (!value) {
    ...
    <div className="form-wrapper">
      ...
      <Form onSubmit={handleSubmit} className="custom-form">
        ...
        <Form.Group className="form-group">
          ...
          {errors.food_Group && <p className="error-text">{errors.food_Group}</p>}
        </Form.Group>

        {[
          { label: 'Energy per 100g (kJ)', value: energi_Kj, setValue: setEnergiKj, error: errors.energi_Kj },
          { label: 'Fat per 100g (g)', value: fett, setValue: setFett, error: errors.fett },
          { label: 'Protein per 100g (g)', value: protein, setValue: setProtein, error: errors.protein },
          { label: 'Carbohydrates per 100g (g)', value: karbohydrat, setValue: setKarbohydrat, error: errors.karbohydrat },
          { label: 'Salt per 100g (g)', value: salt, setValue: setSalt, error: errors.salt },
        ].map((field, index) => (
